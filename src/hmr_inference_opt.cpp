...
void PrintUsage() {
    std::cout
        << "Usage:\n"
        << "  hmr_inference_opt <model.onnx> <video.mp4|image.png> [options]\n\n"
        << "Options:\n"
        ...
        << "  --texture-iters <int>           Opt iterations (default 30)\n"
        << "  --texture-batch <int>           Opt batch size (default 30)\n"
...
int main(int argc, char* argv[]) {
    ...
    for (int i = 3; i < argc; ++i) {
        ...
        if (arg == "--texture-iters" && i + 1 < argc) {
            try {
                options.texture_iters = std::stoi(argv[++i]);
            } catch (const std::exception&) {
                std::cerr << "Invalid texture-iters value." << std::endl;
                return -1;
            }
            continue;
        }
        if (arg == "--texture-batch" && i + 1 < argc) {
            try {
                options.texture_batch = std::stoi(argv[++i]);
            } catch (const std::exception&) {
                std::cerr << "Invalid texture-batch value." << std::endl;
                return -1;
            }
            continue;
        }
